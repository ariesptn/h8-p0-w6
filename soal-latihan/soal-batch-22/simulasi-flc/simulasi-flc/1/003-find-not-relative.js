...
RULES:
 ...
 STORE "result" with the value of arr1
 STORE ""same" with the value of true

  FOR LOOP index 0 to length of arr1
      same = true
      FOR LOOP index 0 to length of arr2
        IF arr1[i] EQUALS to arr2[j] THEN
          same = false
          break
        END IF
      EXIT LOOP
    IF same EQUALS to true THEN
      "result" push arr1[i]
    END IF
  EXIT LOOP
